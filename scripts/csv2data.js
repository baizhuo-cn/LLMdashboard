...
function writeModels(models) {
  ...
  console.log(`[csv2data] Wrote ${TARGET}`);
}
...
(function main() {
  ...
  console.log(`[csv2data] Generating data from ${SOURCE}`);
  ...
  console.log(`[csv2data] Parsed ${dataRows.length} data rows using headers: ${headers.join(', ')}`);
  ...
  models
    .filter((model) => Boolean(KIMI_OVERRIDES[model.id]))
    .forEach((model) => {
      const override = KIMI_OVERRIDES[model.id];
      console.log(
        `[csv2data] Applied Kimi override for ${model.id}: input=${override.input}, output=${override.output}`,
      );
    });

  writeModels(models);
  console.log(`[csv2data] Generated ${models.length} models (updatedAt=${updatedAt}).`);
})();
